...
class Dumphper
{
	...
	static function _dump($source, &$parents = [])
	{
		self::$depth++;
	...
	static function drawArray(&$source, &$parents)
	{
		if (self::isRecursiveArray($source, $parents)) {
			...
			$c = count($source);
			echo '<div class="dumphper-container' . (self::$depth <= self::$max_show_depth ? '' : ' dumphper-closed') . '">';
